...
export default function FamiliesPage() {
  ...
  async function handleDelete(id: string) {
    setDeleting(true);
    await fetch(`/api/families/${encodeURIComponent(id)}`, { method: 'DELETE' });
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        ...
        <Link href="/admin/families/new" className="btn btn-primary">+ New family</Link>
      </div>
              ...
              <tbody>
                {data.map(f => (
                  <tr key={f.id}>
                    ...
                    <td style={{ fontSize: '0.9rem' }}>{personName(f.husband?.name)}</td>
                    <td style={{ fontSize: '0.9rem' }}>{personName(f.wife?.name)}</td>
                    <td style={{ whiteSpace: 'nowrap', fontSize: '0.85rem' }}>{f.marrDate ?? '—'}</td>
                    ...
                    <td>
                      {confirmDeleteId === f.id ? (
                        ...
                        <span style={{ display: 'flex', gap: '0.4rem' }}>
                          <Link
                            href={`/admin/families/${encodeURIComponent(f.id)}/edit`}
                            className="btn btn-secondary btn-sm"
                          >
                            Edit
                          </Link>
