...
export async function POST(req: NextRequest) {
    ...
    await prisma.$transaction(
      batch.map(f =>
        prisma.family.upsert({
          where: { id: f.id },
          create: { id: f.id, husbId: f.husbId, wifeId: f.wifeId, marrDate: f.marrDate, marrPlace: f.marrPlace },
          update: { husbId: f.husbId, wifeId: f.wifeId, marrDate: f.marrDate, marrPlace: f.marrPlace },
        })
      )
    );
  }
  ...
  const importedPersonIds = new Set(people.map(p => p.id));
  for (const family of families) {
    const validChildren = family.children.filter(c => importedPersonIds.has(c));
    await prisma.$transaction([
      prisma.familyChild.deleteMany({ where: { familyId: family.id } }),
      ...validChildren.map(personId =>
        prisma.familyChild.create({ data: { familyId: family.id, personId } })
      ),
    ]);
  }
